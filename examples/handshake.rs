...
fn main() {
    ...
    let monitor = executor.spawn_monitor(TcpStream::connect(epmd_addr.clone())
                                             .and_then(move |socket| {
                                                           client.get_node_info(socket, &peer_name)
                                                       })
                                             .and_then(move |info| if let Some(addr) =
        info.map(|i| SocketAddr::new(epmd_addr.ip(), i.port)) {
        Either::A(TcpStream::connect(addr).and_then(move |socket| {
                                                        let handshake = Handshake::new(&self_node,
                                                                                       &cookie);
